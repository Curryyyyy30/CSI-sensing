...
%% 得到第一种类别的数据文件索引
CurrentPath1     = '.\Direction_judgement\same\';
...
%% 得到第二种类别的数据文件索引
CurrentPath2     = '.\Direction_judgement\opposite\';
...
%% 配置
lenT        = ceil(lengthf1/4 * 0.75) + ceil(lengthf2/4 * 0.75) ;        % 3/4用于训练
lenV        = floor(lengthf1/4 * 0.25) + floor(lengthf2/4 * 0.25) ;     % 1/4用于验证
...
%% 加载第二种类别的验证数据
for i = ceil(lengthf2/4 * 0.75) + 1 : lengthf2/4
    getPath         = LOS2(:,:,i);
    data            = importdata(getPath);
    getPath         = offset2(:,:,i);
    offset          = importdata(getPath);
    ...
    for j = 1 : 8
        DValidation{floor(lengthf1/4*0.25)*8+(i-ceil(lengthf2/4*0.75)-1)*8+j,1}     = squeeze(traindata(:,j)).';
        DALLValidation{floor(lengthf1/4*0.25)*8+(i-ceil(lengthf2/4*0.75)-1)*8+j,1} = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTValidation(:,:,1,floor(lengthf1/4*0.25)*8+(i-ceil(lengthf2/4*0.75)-1)*8+j) = S;
    end
end

%% 生成各类别标签
TTrain          = categorical([ones(ceil(lengthf1/4*0.75)*8,1);2*ones(ceil(lengthf2/4*0.75)*8,1)]);
TValidation     = categorical([ones(floor(lengthf1/4*0.25)*8,1);2*ones(floor(lengthf2/4*0.25)*8,1)]);

% LSTM
layers1 = [
    sequenceInputLayer(1,"Name","sequence","MinLength",200,"SplitComplexInputs",true)
    lstmLayer(300,"Name","bilstm","OutputMode","last")
...
% LSTM-FCN
lgraph = layerGraph();
tempLayers = sequenceInputLayer(1,"Name","sequence","SplitComplexInputs",true);
