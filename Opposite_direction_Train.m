...
%% 得到第一种类别的数据文件索引
CurrentPath1     = '.\Opposite_direction\two_persons\';
...
%% 得到第二种类别的数据文件索引
CurrentPath2     = '.\Opposite_direction\three_persons\';
...
for i = 1 : lengthf2
    ...
    if i < lengthf2 * 0.25 + 1
        LOS2(:,:,i)                 = FileNamePath;
    elseif i < lengthf2 * 0.5  + 1
        NLOS2(:,:,i-lengthf2 *0.25)  = FileNamePath;
    elseif i < lengthf2 * 0.75  + 1
        offset2(:,:,i-lengthf2*0.5) = FileNamePath;
    else
        delay2(:,:,i-lengthf2*0.75) = FileNamePath;
    end
end

%% 得到第三种类别的数据文件索引
CurrentPath3     = '.\Opposite_direction\four_persons\';
...
r1 = 0.75;
r2 = 0.25;
lenT        = ceil(lengthf1/4 * r1) + ceil(lengthf2/4 * r1) + ceil(lengthf3/4 * r1);        % 3/4用于训练
lenV        = floor(lengthf1/4 * r2) + floor(lengthf2/4 * r2) + floor(lengthf3/4 * r2);     % 1/4用于验证
...
%% 加载第一种类别的训练数据
dataindex1 = randperm(ceil(lengthf1/4));
for i = 1:ceil(lengthf1/4 * r1)
    getPath         = LOS1(:,:,dataindex1(i));
    data            = importdata(getPath);
    getPath         = offset1(:,:,dataindex1(i));
    offset          = importdata(getPath);
    getPath         = delay1(:,:,dataindex1(i));
    ...
    for j = 1 : size(data,1)
        DTrain{(i-1)*8+j,1}     = traindata(:,j).';                           % 加载LSTM网络训练数据
        DALLTrain{(i-1)*8+j,1}  = squeeze(data(j,:,:));
        [S(:,:), ~, ~]          = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTTrain(:,:,1,(i-1)*8+j) = S;                                      % 加载RNN网络训练数据
    end
end

%% 加载第一种类别的验证数据
for i = ceil(lengthf1/4 * r1) + 1 : lengthf1/4 
    getPath         = LOS1(:,:,dataindex1(i));
    data            = importdata(getPath);
    getPath         = offset1(:,:,dataindex1(i));
    offset          = importdata(getPath);
    ...
    for j = 1 : 8
        DValidation{(i-ceil(lengthf1/4*r1)-1)*8+j,1}     = squeeze(traindata(:,j)).';
        DALLValidation{(i-ceil(lengthf1/4*r1)-1)*8+j,1}  = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTValidation(:,:,1,(i-ceil(lengthf1/4*r1)-1)*8+j) = S;
    end
end

%% 加载第二种类别的训练数据
dataindex2 = randperm(ceil(lengthf2/4));
for i = 1 : ceil(lengthf2/4 * r1)
    getPath         = LOS2(:,:,dataindex2(i));
    data            = importdata(getPath);
    getPath         = offset2(:,:,dataindex2(i));
    offset          = importdata(getPath);
    getPath         = delay2(:,:,dataindex2(i));
    delay           = importdata(getPath);
    t               = find(offset > offset(1)+2);
    data(:,:,t)     = [];
    offset(:,t)     = [];
    ...
    for j = 1 : 8
        DTrain{ceil(lengthf1/4*r1)*8+(i-1)*8+j,1}     = squeeze(traindata(:,j)).';
        DALLTrain{ceil(lengthf1/4*r1)*8+(i-1)*8+j,1}  = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTTrain(:,:,1,ceil(lengthf1/4*r1)*8+(i-1)*8+j) = S;
    end
end

%% 加载第二种类别的验证数据
for i = ceil(lengthf2/4 * r1) + 1 : lengthf2/4
    getPath         = LOS2(:,:,dataindex2(i));
    data            = importdata(getPath);
    getPath         = offset2(:,:,dataindex2(i));
    offset          = importdata(getPath);
    ...
    for j = 1 : 8
        DValidation{floor(lengthf1/4*r2)*8+(i-ceil(lengthf2/4*r1)-1)*8+j,1}     = squeeze(traindata(:,j)).';
        DALLValidation{floor(lengthf1/4*r2)*8+(i-ceil(lengthf2/4*r1)-1)*8+j,1} = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTValidation(:,:,1,floor(lengthf1/4*r2)*8+(i-ceil(lengthf2/4*r1)-1)*8+j) = S;
    end
end

%% 加载第三种类别的训练数据
dataindex3 = randperm(ceil(lengthf3/4));
for i = 1:ceil(lengthf3/4 * r1)
    getPath         = LOS3(:,:,dataindex3(i));
    data            = importdata(getPath);
    getPath         = offset3(:,:,dataindex3(i));
    offset          = importdata(getPath);
    getPath         = delay3(:,:,dataindex3(i));
    delay           = importdata(getPath);
    t               = find(offset > offset(1)+2);
    data(:,:,t)     = [];
    offset(:,t)     = [];
    ...
    for j = 1 : 8
        DTrain{ceil(lengthf1/4*r1)*8+ceil(lengthf2/4*r1)*8+(i-1)*8+j,1}     = squeeze(traindata(:,j)).';
        DALLTrain{ceil(lengthf1/4*r1)*8+ceil(lengthf2/4*r1)*8+(i-1)*8+j,1}  = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTTrain(:,:,1,ceil(lengthf1/4*r1)*8+ceil(lengthf2/4*r1)*8+(i-1)*8+j) = S;
    end
end

%% 加载第三种类别的验证数据
for i = ceil(lengthf3/4 * r1) + 1 : lengthf3/4
    getPath         = LOS3(:,:,dataindex3(i));
    data            = importdata(getPath);
    getPath         = offset3(:,:,dataindex3(i));
    offset          = importdata(getPath);
    ...
    for j = 1 : 8
        DValidation{floor(lengthf1/4*r2)*8+floor(lengthf2/4*r2)*8+(i-ceil(lengthf3/4*r1)-1)*8+j,1}        = squeeze(traindata(:,j)).';
        DALLValidation{floor(lengthf1/4*r2)*8+floor(lengthf2/4*r2)*8+(i-ceil(lengthf3/4*r1)-1)*8+j,1}     = squeeze(data(j,:,:));
        [S(:,:), ~, ~]    = spectrogram(squeeze(abs(stfttraindata(:,j))), win, R, NFFT, fs);
        DSTFTValidation(:,:,1,floor(lengthf1/4*r2)*8+floor(lengthf2/4*r2)*8+(i-ceil(lengthf3/4*r1)-1)*8+j) = S;
    end
end

%% 生成各类别标签
TTrain          = categorical([2*ones(ceil(lengthf1/4*r1)*8,1);3*ones(ceil(lengthf2/4*r1)*8,1);4*ones(ceil(lengthf3/4*r1)*8,1)]);
TValidation     = categorical([2*ones(floor(lengthf1/4*r2)*8,1);3*ones(floor(lengthf2/4*r2)*8,1);4*ones(floor(lengthf3/4*r2)*8,1)]);
